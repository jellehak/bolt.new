...
export const modificationsRegex = new RegExp(`^<${MODIFICATIONS_TAG_NAME}>[\\s\\S]*?<\\/${MODIFICATIONS_TAG_NAME}>\\s+`, 'g');
export function computeFileModifications(files, modifiedFiles) {
    const modifications = {};
 ...
 */
export function diffFiles(fileName, oldFileContent, newFileContent) {
 ...
 * ```
 */
export function fileModificationsToHTML(modifications) {
    const entries = Object.entries(modifications);
    if (entries.length === 0) {
        return undefined;
    }
    const result = [`<${MODIFICATIONS_TAG_NAME}>`];
